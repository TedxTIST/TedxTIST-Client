...
// Fades history toward #1a1a1a: new = old * 0.55 + #1a1a1a * 0.45
...
interface FBO { fb: WebGLFramebuffer; tex: WebGLTexture; }
...
export default function ThreadBackground({ className }: Props) {
  ...
  useEffect(() => {
    ...
    // ── Compile programs ────────────────────────────────────────────────────
    const lineProg = mkProgram(gl, VS_LINE, FS_LINE);
    const fadeProg = mkProgram(gl, VS_QUAD, FS_FADE);
    const blitProg = mkProgram(gl, VS_QUAD, FS_BLIT);
    ...
    function drawQuad(prog: WebGLProgram, aPosLoc: number) {
      gl!.useProgram(prog);
      gl!.bindBuffer(gl!.ARRAY_BUFFER, quadBuf);
    ...
    // ── Per-thread typed arrays ─────────────────────────────────────────────
    const posData   = new Float32Array(NUM_THREADS * 4);
    const alphaData = new Float32Array(NUM_THREADS * 2);
    ...
    const tlen   = new Float32Array(NUM_THREADS);
    const tspeed = new Float32Array(NUM_THREADS);
    const talpha = new Float32Array(NUM_THREADS);
    ...
    // ── Flow field (identical noise to original) ────────────────────────────
    function noise(x: number, y: number, tv: number): number {
      const s = 0.0025;
      return (
    ...
    function updateFlowField(tv: number) {
    ...
    function onMouseLeave() { hoverTarget = 0; }
    function onTouchMove(e: TouchEvent) {
      e.preventDefault();
      mouse.x = e.touches[0].clientX; mouse.y = e.touches[0].clientY;
      hoverTarget = 1; lastMove = performance.now();
    }
    function onTouchEnd() { hoverTarget = 0; }
    ...
    // ── FBO ping-pong ───────────────────────────────────────────────────────
    let lineFBO!: FBO, histA!: FBO, histB!: FBO;

    function buildFBOs(w: number, h: number) {
      lineFBO = mkFBO(gl!, w, h);
      histA   = mkFBO(gl!, w, h);
      histB   = mkFBO(gl!, w, h);
    ...
    function frame(ts: number) {
      ...
      // Pass 2b: blend new lines on top of faded history
      gl!.enable(gl!.BLEND);
      gl!.blendFunc(gl!.SRC_ALPHA, gl!.ONE_MINUS_SRC_ALPHA);
      gl!.activeTexture(gl!.TEXTURE0);
